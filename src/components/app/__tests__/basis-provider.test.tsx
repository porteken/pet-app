...
import { BasisProvider, readCookieBasis, usePetBasis } from "../basis-provider";
import { BasisToggle } from "../basis-toggle";

vi.unmock("@/components/app/basis-provider");

const { routerRefreshMock, setPetBasisMock } = vi.hoisted(() => ({
  routerRefreshMock: vi.fn<() => void>(),
  setPetBasisMock: vi.fn<() => Promise<void>>(() => Promise.resolve()),
}));

vi.mock("@/lib/actions/actions", () => ({
  setPetBasis: setPetBasisMock,
}));
...
describe("basisProvider", () => {
  ...
  it("should default to max when no cookie is set", () => {
    const { result } = renderHook(() => usePetBasis(), {
  ...
  it("should throw when used outside of a BasisProvider", () => {
    ...
    expect(() => renderHook(() => usePetBasis())).toThrow(
      "usePetBasis must be used within a BasisProvider",
    );
...
describe("basisToggle", () => {
  ...
  it("should toggle the basis, persist the preference, and refresh the router on click", () => {
    ...
    const button = screen.getByRole("button", {
      name: "Switch to daily average PET",
    });
    ...
    expect(
      screen.getByRole("button", { name: "Switch to daily maximum PET" }),
    ).toHaveTextContent("Avg");
    expect(setPetBasisMock).toHaveBeenCalledWith("avg");

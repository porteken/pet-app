...
import { setPetBasis } from "@/lib/actions/actions";
import { useRouter } from "next/navigation";
import * as React from "react";

export const BasisToggle = () => {
  const { basis, setBasis } = usePetBasis();
  const ignorePersistenceError = useIgnorePersistenceError();
  const router = useRouter();

  const isMax = basis === "max";
  const nextBasis = isMax ? "avg" : "max";
  const label = `Switch to daily ${nextBasis === "max" ? "maximum" : "average"} PET`;

  const handleToggle = React.useCallback(() => {
    setBasis(nextBasis);
    void ignorePersistenceError(setPetBasis(nextBasis));
    router.refresh();

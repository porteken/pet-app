...
describe("fetchCityRankingsRows retry and fallback behavior", () => {
  ...
  describe("max basis", () => {
    ...
    it("retries without a season filter when the season column is missing", async () => {
      ...
      mockSelectFromQueue([
        async () => {
          throw missingColumnError("city_rankings_view", "season");
        },
        async () => rows,
      ]);
  ...
  describe("avg basis", () => {
    ...
    it("falls back to the legacy mixed select when the avg columns are missing", async () => {
      ...
      mockSelectFromQueue([
        async () => {
          throw missingColumnError("city_rankings_view", "max_pet_avg");
        },
        async () => rows,
      ]);
    ...
    it("falls back to legacy bounds when avg columns and p10/p90 are both missing", async () => {
      ...
      mockSelectFromQueue([
        ...
        async () => {
          throw missingColumnError("city_rankings_view", "p10_avg");
        },
        async () => rows,
      ]);
...
describe("fetchTrendGraphRows and fetchReferenceGraphRows query building", () => {
  ...
  it("builds and executes the reference graph query", async () => {
    const rows = [{ date: "2020-01-01", location_id: 1, pet: 20 }];
    mockSelectFromQueue([async () => rows]);
